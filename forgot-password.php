...
if (isset($_POST['reset'])){
    ...
    $check = $conn->query("SELECT * FROM users WHERE email='$loginid' OR username='$loginid'" );
    $result = mysqli_fetch_array($check);
    $chk1 = mysqli_num_rows($check);
    ...
    if ($chk1 <= 0){
        ...
        $add = $conn->query("UPDATE users SET token='$gentoken' WHERE email='$getemail' OR username = '$getusername'");
        
        

        $subject = "Reset Password";
        $message = "please <a href='newp-password?token=$gentoken'>click here</a> to reset your password";
        $sender = "From: noreply@payceos.com";
        $send = mail($email,$subject,$message,$sender);
        ...
        if(!$send){
            ...
            Swal.fire({
            title: 'Successful',
            text: 'Password Reset Link has been sent to your registerd email ',
            icon: 'success',
                            ...
                            <div class="auth-box card">
                                <div class="card-block">
                                    ...
                                    <div class="form-group form-primary">
                                        ...
                                        <label class="float-label">Your Email Or Username</label>
                                    </div>
                                    ...
                                    <div class="row m-t-30">
                                        <div class="col-md-12">
                                            <button type="submit" name="reset" class="btn btn-primary btn-md btn-block waves-effect waves-light text-center m-b-20">Proceed</button>
                                        </div>
                                    </div>
                                    <hr/>
