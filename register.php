...
if (isset($_POST['signup'])){
 ...
 if (empty($email) && empty($username) && empty($password) && empty($fullname))   {
    ...
    $error =			 "<script type='text/javascript'>
											...
											Swal.fire({
											title: 'Error!',
											text: 'Sorry! All fieldd Must Be fieled ',
											icon: 'error',
   ...
   $chk1 =  $conn->query("SELECT * FROM users WHERE email='$email' OR username='$username' ");
   ...
   if($chk2 > 0){
    ...
    $error =			 "<script type='text/javascript'>
											...
											Swal.fire({
											title: 'Error!',
											text: 'Sorry! User Already Exist in this Platform Please Login to continue ',
											icon: 'error',
											confirmButtonText: 'Try Again'
											})
    ...
    $insert =  $conn->query("INSERT INTO users (fullname,email,pass,username) VALUES('$fullname','$email','$md5pass','$username')") or die($conn->connect_error);
    
    if ($insert){

        $sid = rand(545854,254558);
